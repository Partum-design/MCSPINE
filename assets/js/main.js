...
function initNavbarScroll() {
  ...
  const onScroll = () => {
    ...
    nav.style.boxShadow = s
      ? "0 14px 40px -20px rgba(2,69,122,0.6)"
      : "0 10px 35px -22px rgba(2,69,122,0.55)";
  };
...
/* ---------- Mo.js surgical burst on nav-link hover ---------- */
  ...
  targets.forEach((el) => {
    ...
    el.addEventListener("mouseenter", () => {
      ...
      new mojs.Burst({
        ...
        count: 6,
        children: {
          fill: [PALETTE.glow, PALETTE.hielo],
          radius: { 4: 0 },
          duration: 600,
          easing: "cubic.out",
        },
...
// Exposed so other modules (catalog form/button) can reuse a click burst
window.mcBurst = function (x, y, opts = {}) {
  if (reduceMotion || !window.mojs) return;
  new mojs.Burst({
    left: x,
    top: y,
    radius: { 0: opts.radius || 50 },
    count: opts.count || 12,
    children: {
      fill: opts.colors || [PALETTE.glow, PALETTE.hielo, "#ffffff"],
      radius: { 7: 0 },
...
/* ---------- Three.js spine background (+ fallback) ---------- */
...
function initThreeBackground() {
  ...
  if (!canvas) return;
  if (reduceMotion || isTouch || window.innerWidth < 900 || !window.THREE) {
    enableFallback();
    return;
  }
  ...
  const key = new THREE.PointLight(0x018abe, 1.2, 100);
  key.position.set(10, 12, 14);
  scene.add(key);
  const fill = new THREE.PointLight(0x97cadb, 0.8, 100);
  ...
  // Build a stylized spine: vertebra discs along a gentle helix curve.
  const SEGMENTS = 26;
  ...
  for (let i = 0; i < SEGMENTS; i++) {
    ...
    const disc = new THREE.Mesh(
      new THREE.TorusGeometry(0.9 - t * 0.28, 0.26, 14, 32),
      i % 4 === 0 ? accentMat : discMat
    );
    const y = (t - 0.5) * 16;
    disc.position.set(Math.sin(t * Math.PI * 2.2) * 1.6, y, Math.cos(t * Math.PI * 2.2) * 1.6);
    disc.rotation.x = Math.PI / 2;
    disc.rotation.z = t * Math.PI * 2.2;
    group.add(disc);
  }
  // floating particles
  ...
  const particles = new THREE.Points(
    pGeo,
    new THREE.PointsMaterial({ color: 0x018abe, size: 0.09, transparent: true, opacity: 0.5 })
  );
  scene.add(particles);
  ...
  // FPS watchdog — degrade to CSS fallback if it can't hold up.
  ...
  function render() {
    ...
    group.rotation.x = mouseY * 0.4 + scrollY * 0.0006;
...
function initScrollReveal() {
  ...
  document.querySelectorAll("[data-sr]").forEach((el) => {
    ...
    sr.reveal(el, { origin, delay, interval: 0 });
  });
}
...
function initHeroReveal() {
  ...
  target.innerHTML = text
    .split("")
    .map((ch) => (ch === " " ? " " : `<span class="reveal-letter">${ch}</span>`))
    .join("");
  ...
  anime({
    ...
    rotateZ: [6, 0],
    duration: 760,
    delay: anime.stagger(28, { start: 280 }),
    easing: "easeOutExpo",
  });
}

/* ---------- Nosotros interconnected nodes ---------- */
function initNodes() {
  ...
  nodes.forEach((node) => {
    ...
    head.addEventListener("click", () => {
      ...
      // close siblings for an elegant single-focus feel
      nodes.forEach((n) => {
  ...
  const p = first.querySelector(".node__panel");
  requestAnimationFrame(() => (p.style.height = "auto"));
}
...
function initContactForm() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    setTimeout(() => {
      window.location.href = `mailto:info@mcspine.mx?subject=${asunto}%20-%20${nombre}&body=${mensaje}`;
    }, 700);
    form.reset();
  });
}
...
/* ---------- Boot ---------- */
function safe(fn) {
  try { fn(); } catch (e) { console.warn("[MC Spine] init failed:", fn.name, e); }
}
function boot() {
  // Safety: never leave reveal content hidden if something below throws.
  document.querySelectorAll("[data-sr]").forEach((el) => (el.style.visibility = "visible"));
  [injectIcons, initLogoPulse, initNavbarScroll, initMobileMenu, initHoverBursts,
   initThreeBackground, initHeroReveal, initNodes, initContactForm, initScrollReveal]
    .forEach(safe);
}
